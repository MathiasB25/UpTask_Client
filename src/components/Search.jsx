...
const Search = () => {
    ...
    const { handleSearch, searchModal, proyects } = useProjects()
    const [ search, setSearch ] = useState('')

    const proyectosFiltrados = search === '' ? [] : proyects.filter( proyect => proyect.name.toLowerCase().includes(search.toLowerCase()))

    return (
        <Transition.Root show = { searchModal } as={ Fragment } afterLeave = { () => setSearch('') }>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={ handleSearch }>
                ...
                >
                    <Combobox
                        ...
                        onChange={ proyect => (window.location = `/projects/${proyect._id}`) }
                    >
